...
# Output Dirs for Probabilities (Using Single Fold 0 as Base, same as V8/V12)
OUTPUT_2D_PROB_DIR = BASE_DIR / 'inference_2d_test_prob'
OUTPUT_3D_LOWRES_PROB_DIR = BASE_DIR / 'inference_3d_lowres_test_prob'

SUBMISSION_DIR = BASE_DIR / 'submission_final_v13_optimized_weights'
ZIP_FILE = BASE_DIR / 'submission_final_v13_20251126.zip'
...
# Soft Voting Weights (V13: Validation-Based Optimization)
# Rationale:
# 1. Myocardium (Label 1): 2D (0.911) > 3D (0.909) in validation. Trust 2D more.
# 2. LV (Label 2): 3D (0.755) > 2D (0.737) in validation. Trust 3D more.
# 3. RV (Label 3): Both are ~0.0 in validation. Trust 2D (1.0) as it's likely empty/sparse.
WEIGHTS = {
    0: {'2d': 0.5, '3d_low': 0.5}, # Background
    1: {'2d': 0.6, '3d_low': 0.4}, # Myocardium: Boost 2D (was 0.3)
    2: {'2d': 0.3, '3d_low': 0.7}, # LV: Keep 3D dominance (was 0.3/0.7)
    3: {'2d': 1.0, '3d_low': 0.0}  # RV: Trust 2D (likely empty)
}
...
def run_ensemble():
    print(f"Running V13 Ensemble (Optimized Weights)...")
    ...
    for case_id in test_cases:
        ...
        if not prob_3d_file.exists(): prob_3d_file = OUTPUT_3D_LOWRES_PROB_DIR / f"{case_id}.npz"
