...
SUBMISSION_DIR = BASE_DIR / 'submission_final_v16_updated'
ZIP_FILE = BASE_DIR / 'submission_final_v16_updated_20251128.zip'

# Weights (Updated V16)
# Myo/LV(Full:0.7), RV(2D:0.65)
# Fullres = (F0 + F1) / 2
# Lowres = 5-Fold
# 2D = 5-Fold
WEIGHTS = {
    0: {'2d': 0.4, 'low': 0.3, 'full': 0.3},
    1: {'2d': 0.0, 'low': 0.3, 'full': 0.7}, # Myo: Trust Fullres (0.7) + Lowres (0.3)
    2: {'2d': 0.0, 'low': 0.3, 'full': 0.7}, # LV: Trust Fullres (0.7) + Lowres (0.3)
    3: {'2d': 0.65, 'low': 0.35, 'full': 0.0}  # RV: Trust 2D (0.65) + Lowres (0.35)
}
...
def run_ensemble():
    print(f"=== Running V16 Updated Ensemble ===")
