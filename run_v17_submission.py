...
# Input Probabilities
PROB_2D_5FOLD_DIR = BASE_DIR / 'inference_2d_5fold_prob'
...
SUBMISSION_DIR = BASE_DIR / 'submission_final_v17_optimized'
ZIP_FILE = BASE_DIR / 'submission_final_v17_optimized_20251128.zip'

# Weights (V17 Optimized)
# Strategy: Boost the "Excellent" models.
# Myo/LV: Fullres is best. Increase to 0.85.
# RV: 2D is best (Fullres failed in validation). Increase to 0.85.
# Lowres: Reduced to 0.15 as a stabilizer.

WEIGHTS = {
    0: {'2d': 0.4, 'low': 0.3, 'full': 0.3}, # Background: Balanced
    1: {'2d': 0.0, 'low': 0.15, 'full': 0.85}, # Myo: Trust Fullres (0.85)
    2: {'2d': 0.0, 'low': 0.15, 'full': 0.85}, # LV: Trust Fullres (0.85)
    3: {'2d': 0.85, 'low': 0.15, 'full': 0.0}  # RV: Trust 2D (0.85)
}
...
def run_ensemble():
    print(f"=== Running V17 Optimized Ensemble ===")
    print("Components: 3D Fullres (F0+F1) + 3D Lowres (5-Fold) + 2D (5-Fold)")
    print("Weights: Myo/LV (Full:0.85), RV (2D:0.85)")
    ...
    for i, case in enumerate(cases):
        ...
        p2d = load_prob(PROB_2D_5FOLD_DIR, case)
